 ...
 * Author: brunomont
 * Date: 2022-10-25
 * Source: https://github.com/brunomaletta/Biblioteca/blob/master/Codigo/Estruturas/SegTree/SegTreeBeats.cpp
 ...
 * updatemin(a, b, x) turn v[i] <- min(v[i], x), for each i in [a, b]
 * updatemax do the same operation with max, and updatesum add x
 * for each i in [a, b]
 * Complexity
 * build - O(n)
 * query - O(log(n))
 * update - O(log^2 (n)) amortizado (se nao usar updatesum, fica log(n) amortizado)
*/
#define f first
#define s second
